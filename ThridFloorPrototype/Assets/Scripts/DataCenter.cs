...
public class DataCenter : MonoBehaviour
{
    ...
    {
        string url = $"http://116.204.183.220:3000/get/progress";
    ...
    {
        string url = "http://116.204.183.220:3000/set/currentprogress";
        ...
        StartCoroutine(ApiManager.Instance.PostRequest(url, jsonData, (response) =>
        {
            if (!string.IsNullOrEmpty(response))
            {
    ...
    {
        SendProgress scoreData = new SendProgress
        {
            Region = region,
            Stage = stage,
            Score = score,
        ...
        string url = "http://116.204.183.220:3000/set/score";

        StartCoroutine(ApiManager.Instance.PostRequest(url, jsonData, (response) =>
        {
            if (!string.IsNullOrEmpty(response))
            {
                Debug.Log("Score data sent successfully: " + response);

                var rewardResponse = JsonUtility.FromJson<RewardResponse>(response);

                if (rewardResponse != null && rewardResponse.Reward != null)
                {
                    if (rewardImage != null)
                    {
                        DataCenter.Instance.SetRewardImage(rewardImage, rewardResponse.Reward.ItemType);
                    }

                    if (rewardText != null)
                    {
                        rewardText.text = rewardResponse.Reward.Quantity > 0 ? $"x{rewardResponse.Reward.Quantity}" : "x0";
                    }
                }
            ...
            {
                Debug.LogError("Failed to send score data");
            }
        }));
    }


    public void SetRewardImage(Image rewardImg, string typeReward)
    {
        switch(typeReward)
        {
            case "Gold":
                rewardImg.sprite = itemTypeGold; 
                break;
            case "Gem":
                rewardImg.sprite = itemTypeGold;
                break;
    ...
    {    
        string apiUrl = "http://116.204.183.220:3000/get/equipmentshop";


        yield return StartCoroutine(ApiManager.Instance.GetRequest(apiUrl, (response) =>
        {
            if (!string.IsNullOrEmpty(response))
            {
